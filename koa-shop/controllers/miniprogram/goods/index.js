...
async function detailAction(ctx) {
     ...
     * */ 
    const attribute = await mysql('nideshop_goods_attribute').column('nideshop_goods_attribute.value', 'nideshop_attribute.name')
    .leftJoin('nideshop_attribute', 'nideshop_goods_attribute.attribute_id', 'nideshop_attribute.id').where({
    ...
    // 大家都在看
    const recommendList = await mysql('nideshop_goods').where({
    ...
    // 判断购物车中有几件商品
    const cartNumber = await mysql('nideshop_cart').where({
        'user_id': openId
